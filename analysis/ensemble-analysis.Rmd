---
title: 'Final round ensembled model scoring'
...
p<-lapply(ens_names, function(x){
  cors <- cor(ensemble.sc1$gold, ensemble.sc1[[x]], method = 'spearman')
  ggplot(data = ensemble.sc1) +
    geom_point(aes(x = gold, y = !!sym(x))) +
    ggtitle(glue::glue("spearman = {cors}"))
})
...
p<-lapply(ens_names, function(x){
  cors <- cor(ensemble.sc2$score, ensemble.sc2[[x]], method = 'spearman') 
...
p<-lapply(ens_names, function(x){
  cors <- cor(ensemble.sc3$score, ensemble.sc3[[x]], method = 'spearman') 
...
lvls <- colnames(results.sc1)[-2]
original_sc1_plot_data$model <- factor(original_sc1_plot_data$model, levels = lvls)
  
ens_violin_a <-ggplot(data = original_sc1_plot_data) +
  geom_violin(aes(x = model, y = prediction),cex = 0.2) +
  theme_bw() +
theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
...
When you average team RYM in, the distribution starts to look more like the gold/top performers: 
...
ens_violin_b <- ggplot(data = ensemble_plot_data) +
  geom_violin(aes(x = model, y = prediction),cex = 0.2) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
...
My guess is that this is a fluke, probably caused by their generally more conservative predictions coincidentally helping reshape the overall distribution of the ensemble to better mirror that of the gold standard (at least, that's what the third plot I attached seems to demonstrate). SC1 also has substantially lower N than SC2/3, so maybe a better chance of getting lucky in this way. 

## supplemental figure 

```{r echo=TRUE, message=FALSE}

cowplot::plot_grid(ens_violin_a,ens_violin_b, labels = "AUTO")

ggsave("../figures/figure_s5.pdf", width = 9, height = 5)
...
# Rearrange SC1 ensemble. 

To check for coding errors (in addition to inspecting the code itself), lets reorganize the ensemble, "doping in" RYM far earlier to see if we get the same effect. We do: 
...
new_order <- c("Team Shirin", "Hongyang Li and Yuanfang Guan", "csabaibio", "Nc717", "zs_aicoe",
               "RYM", "NAD", "Gold Therapy" ,"Alpine Lads",  "CU_DSI_RA2_Challenge",
               "kichuDL", "Aboensis V", "Zbigniew Wojna", "RA2 Baseline Model")

results.sc1 <- submissions.sc1[,c("gold", "weight", new_order)]
...
ggplot2::ggsave("ra2_ensemble.pdf", height = 5, width = 15, units = "in")
```
