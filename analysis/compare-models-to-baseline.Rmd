...
Before bootstrapping, rearrange the teams by rank, where left = better performance and right = not-as-great performance.
...
Use our `challengescoring` package to compute Bayes factors using a matrix of scores, setting the `refPredIndex` as the number of the column that contains the baseline/comparator prediction (the reference prediction). Because the baseline scores are less than the top performing scores, the Bayes factor will need to be inverted (`invertBayes=TRUE`).

```{r echo=TRUE, message=FALSE, warning=FALSE}
# Baseline: RA2 Baseline Model (columns 15, 12 and 10)
bayes_baseline.sc1 <- computeBayesFactor(boot.sc1, refPredIndex=15, invertBayes=T) %>%
  as_tibble(rownames = "submission") %>%
  rename(bayes = value)

bayes_baseline.sc2 <- computeBayesFactor(boot.sc2, refPredIndex=12, invertBayes=T) %>%
  as_tibble(rownames = "submission") %>%
  rename(bayes = value)

bayes_baseline.sc3 <- computeBayesFactor(boot.sc3, refPredIndex=10, invertBayes=T) %>%
  as_tibble(rownames = "submission") %>%
  rename(bayes = value)
```
...
plot_results <- function(results, bayes, subchallenge) {
  results %>%
    ...
    mutate(bayes_category=case_when(
      submission == "RA2 Baseline Model" ~ "Reference",
      bayes<=3 ~ "<3",
      bayes>=3 & bayes <5 ~ "3-5",
...
plot_results(boot.sc1, bayes_baseline.sc1, "SC1")
plot_results(boot.sc2, bayes_baseline.sc2, "SC2")
plot_results(boot.sc3, bayes_baseline.sc3, "SC3")
```
