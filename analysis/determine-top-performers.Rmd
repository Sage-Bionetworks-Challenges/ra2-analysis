...
Compute Bayes factor again, but this time, use each column as the point of reference. Some tweaking to the computation will be required, as the reference index is no longer the best, which in consequence, means the inversion of K will be dependent on which submission it is being compared with.
...
Visuals are a bit unclear between the top performers and the second-top performers for SC1 and SC3, so zooming in on the plots a little bit.
...
Additionally, test the significance by performing a pairwise t-test across all predictions.
...
pairwise_ttest(boot.sc1)
```

### Subchallenge 2

```{r echo=FALSE}
pairwise_ttest(boot.sc2)
```

### Subchallenge 3

```{r echo=FALSE}
pairwise_ttest(boot.sc3)
```
