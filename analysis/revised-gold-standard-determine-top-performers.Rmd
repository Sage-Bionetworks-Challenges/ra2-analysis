...
##filtered out Sha2256:6cb.... because this was the wrong baseline model; we re-ran with the correct baseline 
...
query <- syn$tableQuery(
  ...
  FROM syn22236264 WHERE status = 'ACCEPTED' AND 
   ...
   ## remove participants that did not submit writeup to final round (3408914 and 3399952)
...
```{r echo=TRUE, message=FALSE}
sc1 <- plot_results(boot.sc1, bayes_top.sc1, "SC1")
sc1
...
```{r echo=TRUE, message=FALSE}
sc2 <- plot_results(boot.sc2, bayes_top.sc2, "SC2")
sc2
```
...
```{r echo=TRUE, message=FALSE}
sc3 <- plot_results(boot.sc3, bayes_top.sc3, "SC3")
sc3
plot_results(boot.sc3[,1:6], bayes_top.sc3, "SC3")
```

## Recalculate Bayes against Baseline model
...
bayes_baseline.sc1 <- computeBayesFactorWhereRefIsNotBest(boot.sc1, refPredIndex=12, invertBayes=F) %>%
  as_tibble(rownames = "submission") %>%
  rename(bayes_baseline = value)

bayes_baseline.sc2 <- computeBayesFactorWhereRefIsNotBest(boot.sc2, refPredIndex=14, invertBayes=F) %>%
  as_tibble(rownames = "submission") %>%
  rename(bayes_baseline = value)

bayes_baseline.sc3 <- computeBayesFactorWhereRefIsNotBest(boot.sc3, refPredIndex=13, invertBayes=F) %>%
  as_tibble(rownames = "submission") %>%
  rename(bayes_baseline = value)
...
###Make results figure for paper
Tentatively Figure 3. 

```{r}

plot_results_figure <- function(results, bayes, bayes_2, subchallenge) {
  foo <-results %>%
    as_tibble() %>%
    gather(submission, bs_score) %>%
    left_join(bayes) %>%
    left_join(bayes_2) %>% 
    mutate(bayes = case_when(bayes == Inf ~ 99999,
                     bayes != Inf ~ bayes)) %>% 
    mutate(bayes_baseline = case_when(bayes_baseline == Inf ~ 99999,
                     bayes_baseline != Inf ~ bayes_baseline)) %>% 
    mutate(bayes_category=case_when(
      bayes == 0 ~ "Reference",
      bayes<3 ~ "<3",
      bayes>=3 & bayes <=20 ~ "3-20",
      bayes>20 & submission != "RA2 Baseline Model" ~ ">20",
      bayes>20 & submission == "RA2 Baseline Model" ~ "Baseline Model")) %>% 
    mutate(submission = fct_reorder(submission, -bs_score, .fun = mean))
   ...
   a <-  ggplot(foo, aes(
    ...
    scale_color_manual(values = c(
      ...
      "3-20" = '#53B3CB', 
      ">20" = "#1D3461",
      "Baseline Model" = '#FFD447'),
      name = "Bayes Factor") +
    theme(legend.position = "none")+
    coord_flip() +
    labs(x="Team", y=paste("Bootstrapped", subchallenge, "Score"))
    
  b<- ggplot(foo %>% select(-bs_score) %>% distinct, aes(
      x=submission,
      y=bayes,
      fill=bayes_category
    )) +
    ...
    labs(x="", y=paste("Bayes (vs top)")) +
    scale_y_continuous(breaks = c(0, 3, 5, 10, 20),
                   labels = c(0, 3, 5, 10, 20))
    
   
   c<- ggplot(foo %>% select(-bs_score) %>% distinct, aes(
      x=submission,
      y=bayes_baseline,
      fill=bayes_category
    )) +
    ...
    scale_fill_manual(values = c(
      ...
      ">20" = "#1D3461",
      "Baseline Model" = '#FFD447'),
      name = "Bayes Factor (vs top)") +
    theme(axis.text.y=element_blank(),axis.ticks.y = element_blank()) +
    coord_flip(ylim = c(0,20)) +
    labs(x="",y=paste("Bayes (vs baseline)")) +
    scale_y_continuous(breaks = c(0, 3, 5, 10, 20),
                   labels = c(0, 3, 5, 10, 20))
  ...
  c + theme(legend.box.margin = margin(0, 0, 0, 12))
   ...
   cowplot::plot_grid(a,b,c+theme(legend.position="none"),legend, nrow = 1, rel_widths = c(3,1,1,1))
    
}


plt <- plot_results_figure(boot.sc1, bayes_top.sc1, bayes_baseline.sc1, "SC1")
plt2 <- plot_results_figure(boot.sc2, bayes_top.sc2, bayes_baseline.sc2, "SC2")
plt3 <- plot_results_figure(boot.sc3, bayes_top.sc3, bayes_baseline.sc3, "SC3")

cowplot::plot_grid(plt, plt2, plt3, nrow = 3, labels = "AUTO")

ggsave("../figures/Figure3.pdf", height = 12, width = 10, scale = 1)

```


### Manuscript figure content
...
ggsave("figure_3_bootstrapping.svg")
ggsave("figure_3_bootstrapping.pdf")
ggsave("figure_3_bootstrapping.png")
...
```{r echo=FALSE}
sc1_pval_plot<-sc1.pvals %>% 
  ggplot(data = ., aes(x=team_a, y=team_b, fill=fdr)) + 
  geom_tile() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  labs(x = 'Team', y = 'Team') + 
  scale_fill_gradient2("FDR adjusted p-value") 
...
legend <- cowplot::get_legend(
  # create some space to the left of the legend
  sc1_pval_plot + theme(legend.box.margin = margin(0, 0, 0, 12))
)

plt <- cowplot::plot_grid(sc1_pval_plot + theme(legend.position="none"),
                   sc2_pval_plot + theme(legend.position="none"),
                   sc3_pval_plot + theme(legend.position="none"),
                   nrow = 1, 
                   labels = "AUTO")

plt_2 <- cowplot::plot_grid(plt, legend, nrow = 1, rel_widths = c(3,0.5))
plt_2

ggsave('supplemental_figure_final_submission_pairwise_p_values.pdf')
ggsave('supplemental_figure_final_submission_pairwise_p_values.png')
