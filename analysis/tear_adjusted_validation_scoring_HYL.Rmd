---
title: 'TEAR Validation - *adjusted* dataset - prediction scoring, v2'
...
lifelines <- reticulate::import('lifelines')
...
query  <- syn$tableQuery(
  "SELECT id, prediction_fileid, orginial_submitterid as original_submitterid, createdOn,
    ...
    FROM syn25875469 WHERE status = 'ACCEPTED' AND 
...
ggsave("tear_adjusted_dual_gold_standards_validation_hyl.pdf", width = 10, height = 4)
...
valid_vs_original <- mean_validation %>% 
  ...
  filter(team != 'vladyorsh') %>%  ## don't have validation predictions for this submitter b/c dq for no writeup
...
ggsave("tear_adjusted_vs_final_round_validation_hyl.pdf", width = 12, height = 4)
...
ggsave("tear_adjusted_vs_final_round_lineplot_validation_hyl.pdf", width = 12, height = 4)


```

# Compute c-index for predictions

```{r}

c_indices <- valid_vs_original %>% 
  group_by(sc) %>% 
  summarize(c_ind = c_index(score_orig, mean_validation_score) %>% round(3)) %>% 
  rename("Concordance Index" = c_ind, "Subchallenge" = sc)
