---
title: 'TEAR Validation dataset prediction scoring'
...
We have an independent dataset, TEAR, that also has hand and foot RA images. We tested the 15 final round docker containers on an independent TEAR cohort of 50 individuals to assess which methods are most stable in new datasets. 
...
query <- syn$tableQuery(
  "SELECT id, prediction_fileid, original_submitterid, createdOn,
    sc1_weighted_sum_error AS sc1, 
    sc2_joint_weighted_sum_rmse AS sc2, 
    sc3_joint_weighted_sum_rmse AS sc3
  FROM syn25186123 WHERE status = 'ACCEPTED' AND 
    original_submitterid <> 3408914 AND
    original_submitterid <> 3399952 AND
...
ggplot(data = all_scores) +
  ...
  ggrepel::geom_text_repel(aes(x= score_1, y = score_2, label = team), size = 2.5, force = 10, box.padding = 0.5, min.segment.length = 0.25, direction = "both", segment.colour = "darkgray") +
  facet_wrap(category ~ .) + 
  labs(x = "Gold standard one", y = "Gold standard two") +
  theme_bw()

ggsave("tear_dual_gold_standards.pdf", width = 10, height = 4)
...
valid_vs_original <- mean_validation %>% 
  ...
  mutate(sc = case_when(category == "sc1_weighted_sum_error" ~ "SC1",
                   category == "sc2_joint_weighted_sum_rmse" ~ "SC2",
                   category == "sc3_joint_weighted_sum_rmse" ~ "SC3",))

ggplot(data = valid_vs_original) +
  geom_point(aes(x = score_orig, y = mean_validation_score)) +
  ggrepel::geom_text_repel(aes(x= score_orig, y = mean_validation_score, label = team), size = 2.5, force = 10, box.padding = 0.5, min.segment.length = 0.25, direction = "both", segment.colour = "darkgray") +
  facet_wrap(. ~ category, nrow = 1, scales = 'free') + 
  labs(x = "Final round score", y = "Post-challenge validation score (mean)") +
  theme_bw()

ggsave("tear_vs_final_round.pdf", width = 10, height = 4)
...
ggplot(data = valid_vs_original_gathered,
       aes(x = fct_relevel(label, ord), y = score, color = team)) +
  geom_point() +
...
ggsave("tear_vs_final_round_lineplot.pdf", width = 10, height = 4)
